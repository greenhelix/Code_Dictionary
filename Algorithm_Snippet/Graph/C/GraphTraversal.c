...
void DFS(Vertex* V){  // 시작점이 될 정점을 매개변수로 받습니다.
...
void BFS(Vertex* V , LinkedQueue* Queue){
    ...
    while(!LQ_IsEmpty(Queue)){ // 큐가 다 비어질 때까지 계속 돌린다~
        ...
        while(E!= NULL){ // 큐에서 빼낸 그 노드에 연결된 간선들 다 없어질때까지 반복

            V = E -> Target; // 일단 하나씩 다 본다.

            if(V != NULL && V->Visited == NotVisited){ // 미방문이고, 있다면 조건 성립!
